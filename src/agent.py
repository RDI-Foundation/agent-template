...
class Agent:
    def __init__(self):
        ...
        self.required_roles = ["pro_debater", "con_debater"]
        self.required_config_keys = ["topic", "num_rounds"]
    ...
    async def run(self, input_text: str, updater: TaskUpdater) -> None:
        try:
            req: EvalRequest = EvalRequest.model_validate_json(input_text)
            ok, msg = self.validate_request(req)
            if not ok:
                await updater.reject(
                    new_agent_text_message(
                        f"Request validation failed: {msg}", updater.context_id
                    )
                )
                return
            ...
            await updater.reject(
                new_agent_text_message(
                    f"Invalid request format: {str(e)}", updater.context_id
                )
            )
            return

        debate = await self.orchestrate_debate(
            req.participants, req.config["topic"], req.config["num_rounds"], updater
        )
        ...
        await updater.update_status(
            TaskState.working,
            new_agent_text_message(
                f"Debate orchestration finished. Starting evaluation."
            ),
        )
        logger.info("Debate orchestration finished. Evaluating debate.")
        debate_eval: DebateEval = await self.judge_debate(
            req.config["topic"], debate_text
        )
